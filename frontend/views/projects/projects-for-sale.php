...
<section class="news">
    <div class="block">
        ...
        <nav class="bookmarks" id="bookmarks">
            <a href="<?=Url::to(['/projects', 'slug' => null])?>" class="<?= $slug == null || $slug == 'articles' ? 'active' : ''?>">Всі проєкти</a>
            <?php if ($tabs): ?>
                <?php foreach ($tabs->parents as $parent): ?>
                    <a href="<?=Url::to(['/projects/projects-for-sale', 'slug' => $parent->slug])?>" class="<?= $parent->slug === $slug ? 'active' : ''?>"><?=$parent->getTitle($parent->id)?></a>
                <?php endforeach; ?>
            <?php endif; ?>
        </nav>
