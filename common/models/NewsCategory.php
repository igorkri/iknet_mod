...
//            ],
    ...
    public function getParents(){
        return $this->hasMany(NewsCategory::class, ['parent_id' => 'id'])->orderBy('order ASC');
    }
    public function getParents2(){
        return $this->hasMany(NewsCategory::class, ['id' => 'parent_id']);
    }
...
//        die;
    ...
    static function getTabNews($slug){
        $cat_1 = NewsCategory::find()
            ->with(['parents'])
            ->where(['slug' => $slug])
            ->orderBy('order DESC')
            ->one();
        if(isset($cat_1->parents) && count($cat_1->parents) <= 0){
            $cat_2 = NewsCategory::find()
                ->with(['parents'])
